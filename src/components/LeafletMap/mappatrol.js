...
const MapContainerComponent = ({
  ...
  useEffect(() => {
    ...
    if (data) {
      console.log("Incoming scatter: ", data)
      ...
      data.forEach((marker, index) => {
        const customIcon = L.icon({
          iconUrl: marker.iconUrl, // Use the provided custom icon URL
          iconSize: [30, 46], // Adjust based on your icon size
        });
        // console.log(marker)
        const { lat, lng, label, icon } = marker;
    ...
    <>
      ...
      <Row>
        <Col span={24}>
          ...
          <div ref={mapRef} style={{ height: "706px", width: "100%" }} />
