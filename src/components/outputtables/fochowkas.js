...
import fochowkasdata from "../../data/fochowkasdata.json";
import { fetchStaticSpecialReportData } from "../../redux/thunks/patroltypeBasedData.js";

function FOChowkas({apidata}) {
  const router = useRouter();
  const dispatch = useDispatch();
  const { data, isLoading } = useSelector(
    (state) => state.fetchStaticSpecialReportData
  );
  const [searchData, setSearchData] = useState({ ssr_table: 'FO CHOWKAS' });
