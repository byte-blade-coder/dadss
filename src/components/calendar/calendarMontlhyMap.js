    ...
    const response = uniqueShips.map((ship) => {
      ...
      const contributionsByMonth = Array.from({ length: 12 }, (_, i) => {       
        ...
        //  )
          // .filter(
          //   (entry) =>
          //   {  dayjs(entry.date).isSameOrAfter(monthStart) || dayjs(entry.date).isAfter(monthStart)
          //   //   ,
      ...
      //     return contributions;
       ...
       // Filter entries that are within the current month range
    const filteredEntries = shipEntries.filter((entry) => {
      const entryDate = dayjs(entry.date);
      const isAfterMonthStart = entryDate.isSameOrAfter(monthStart);
      const isBeforeMonthEnd = entryDate.isBefore(monthEnd.add(1, 'day')); // Add 1 day to include the last day of the month

      console.log("Is entry within the month?", isAfterMonthStart, isBeforeMonthEnd);
      console.log("Entry:", entry);
      
      return isAfterMonthStart && isBeforeMonthEnd;
    });

    console.log("Filtered Entries for " + ship + " in month " + (i + 1) + ":", filteredEntries);

    // Calculate total contributions for this month using reduce
    const contributions = filteredEntries.reduce((acc, entry) => {
      console.log("Accumulating contribution for entry:", entry);
      return acc + entry.contributions; // Ensure you're returning the new accumulated value
    }, 0);

    console.log("Total Contributions for " + ship + " in month " + (i + 1) + ":", contributions);
    return contributions;
