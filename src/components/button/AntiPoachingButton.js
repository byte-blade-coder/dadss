...
const AntiPoachingButton = ({ onDataLoad }) => {
  // State variables
 ...
 // console.log(cleanedCsvData);
  ...
  const handleFileChange = (e) => {
    ...
    reader.onload = (event) => {
      ...
      const requiredColumns = [
        "DATE", "YEAR", "LATITUDE", "LONGITUDE", "APPREHENDED BY", "BOAT NAME", "COUNTRY", "CREW",
                "CREW NAME", "NETS SEIZED (Quantity)", "FISH CATCH SEIZED (Quantity)"
      ];
  ...
  const handleUpload = async (e) => {
    ...
    try {
      ...
      dispatch(addCOIshipUploadData(formData));
    } catch (error) {
  ...
  const handleLoad = async () => {
    ...
    if (file.name !== "ANTI POACHING.csv") {
    ...
    try {
      ...
      formData.append('table', "Anti-Poaching");
      formData.append('sheet', "ANTI POACHING");
     ...
     // Check if the response status is successful
      if (response.status === 200 || response.status === 201) {
        console.log("Got this reponse: ", response)
        ...
        if (onDataLoad) {
          console.log("Response On data load: ", response.data)
...
export default AntiPoachingButton;
