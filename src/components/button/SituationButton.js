...
const SituationButton = ({ onDataLoad }) => {
  ...
  const { isLoading } = useSelector((state) => state.situationreport);
  ...
  const handleUpload = async (e) => {
    ...
    try {
      ...
      dispatch(addSituationUploadData(formData));
    } catch (error) {
  ...
  const handleLoad = async () => {
    ...
    const apiUrl = `${process.env.NEXT_PUBLIC_MSA_BACKEND_API}/sit_report_json`;
