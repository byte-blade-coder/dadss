...
import { addJmisLostReportUploadData } from "../../redux/thunks/jmisLostReportUploadData";
import { toast } from "react-toastify";
import "react-toastify/dist/ReactToastify.css";
import axios from "axios";

const JmisLostReportButton = ({ onDataLoad }) => {
  // State variables
  ...
  const { isLoading } = useSelector((state) => state.lostreport);
  ...
  const handleFileChange = (e) => {
    ...
    reader.onload = (event) => {
      ...
      console.log("CSV parsedData: ", parsedData)
  ...
  const handleUpload = async (e) => {
    ...
    try {
      ...
      dispatch(addJmisLostReportUploadData(formData));
    } catch (error) {
  ...
  const handleLoad = async () => {
    ...
    const apiUrl = `${process.env.NEXT_PUBLIC_MSA_BACKEND_API}/lp_json`;
...
export default JmisLostReportButton;
