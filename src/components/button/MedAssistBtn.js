...
const MedAssistBtn = ({ onDataLoad }) => {
  ...
  const { isLoading } = useSelector((state) => state.fetchSARData);
 ...
 // console.log(cleanedCsvData);
  ...
  const handleFileChange = (e) => {
    ...
    reader.onload = (event) => {
      ...
      //   'UNIT', 'DATE', 'BOAT APPREHENDED', 'LATITUDE', 'LONGITUDE', 'REG NO', 'CREW NAME',
      //   'CREW CNIC', 'CREW', 'VILLAGE'
      // ];
  ...
  const handleLoad = async () => {
    ...
    if (!file) {
      setFileName("");
      setUploadMessage("Upload failed. Please select a CSV/XLSX file.");
      alert("Please select a CSV/XLSX file.");
    ...
    try {
      ...
      formData.append('table', "MEDICAL ASSISTANCE");
      formData.append('sheet', "MEDICAL ASSISTANCE");
