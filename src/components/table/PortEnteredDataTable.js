...
function MissionDetailDataTable(props) {
  ...
  const missionDetailDataEdited = (key) => {
    ...
    setMissionDetail((previous) => {
      ...
      console.log("previous", newItems, previous)
      newItems[key] = newEdited;
      console.log(newItems[key], key)
      return newItems;
    });
  ...
  const onMissionDetailDataFinish = async () => {
    ...
    if (validatedValues) {
      ...
      showToastSuccess(`${title} table data added`);
      setShowInputs({ ...showInputs, missionDetailColumns: false });
      missionDetailForm.resetFields();
    }
  };
  ...
  const missionDetailsDataColumns = [
    ...
    {
      title: "Location/Number of Berth",
    ...
    },
    {
      ...
      render: (text, record, index) => {
        ...
        if (!showInputs.missionDetailColumns) {
          // console.log("!showInputs.missionDetailColumns")
          if (
            missionDetail.length &&
            !isMissionDetailDataEditing(index) &&
            showButtons
          ) {
            //when there is data/rows in table
            // console.log("if", !isMissionDetailDataEditing(index), missionDetail.length  )
            return (
              <IconsStylingWrap>
                <MdModeEditOutline
                  className="editIcon"
