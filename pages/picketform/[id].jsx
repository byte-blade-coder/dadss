...
function Details() {
  ...
  useEffect(() => {
    if (data) {
      ...
      if (data?.tripDetails) {
        ...
        if(data.tripDetails[0]?.rvt_tripstatus === "completed")
        {
          ...
          tripForm.setFieldsValue({
            // ...data.tripDetails,
            rvt_depdate: data.tripDetails[0]?.rvt_depdate? dayjs(data.tripDetails[0]?.rvt_depdate).format("DD-MM-YYYY HH:mm:ss") : null,
            // rvt_depdate: data.tripDetails[0]?.rvt_depdate? data.tripDetails[0]?.rvt_depdate : null,
            datetime: data.tripDetails[0]?.rvt_depdate? dayjs(data.tripDetails[0]?.rvt_depdate).format("DD-MM-YYYY HH:mm:ss") : null,
            rvt_arrivaldate: data.tripDetails[0]?.rvt_arrivaldate? dayjs(data.tripDetails[0]?.rvt_arrivaldate).format("DD-MM-YYYY HH:mm:ss") : null,
            datetime1: data.tripDetails[0]?.rvt_arrivaldate? dayjs(data.tripDetails[0]?.rvt_arrivaldate).format("DD-MM-YYYY HH:mm:ss") : null,
  ...
  const handleFishingSave = async () => {
    ...
    const finalData = {
      rvt_rv_key: id,
      rvt_depdate: tripValues.rvt_depdate ? vesselStatus==="departure" ? dayjs(tripValues.rvt_depdate).format('YYYY-MM-DDTHH:mm:ssZ') : data.tripDetails[0]?.rvt_depdate : null,
    ...
    const newFinal = {
      data: finalData,
      key: vesselStatus==="return" ? data.tripDetails[0]?.rvt_key : null,
      navigation: router,
    };
  ...
  return (
    <>
      ...
      <VesselTripPicketForm disabled={false} form={tripForm} vesselStatus={vesselStatus}></VesselTripPicketForm>
