...
import {ais_type_summary,fishing_vessel_types } from "../../../src/helper/dropdown";
...
//   () => import("../../../src/components/barchart/DivergingBarPlotGrouped.js"),
//   {
//     ssr: false,
//     loading: () => (
//       <LoadingOutlined style={{ color: "#012169", fontSize: "40px" }} />
...
//currentGroup
  ...
  useEffect(() => {
    ...
    if(visual==="line")
    {
      if(apiFilter==="harbor")
      {
        subtitle = `Aggregated fishing vessel data for the specified date range, providing insights into the number of vessels present at KARACHI, PORT QASIM, and GWADAR ports.`;
        setSubTitle(subtitle);
      }
      else if(apiFilter==="type")
      {
        subtitle = `Aggregated fishing vessel data for the specified date range, providing insights into the vessel count that is categorized by vessel type.`;
        setSubTitle(subtitle);
      }
    ...
    else if(visual==="bar")
    {
      if(apiFilter==="harbor")
      {
        subtitle = `Fishing vessel data for the specified date range, providing insights into the number of vessels present at KARACHI, PORT QASIM, and GWADAR ports.`;
        setSubTitle(subtitle);
      }
      else if(apiFilter==="type")
      {
        subtitle = `Fishing vessel data for the specified date range, providing insights into the vessel count that is categorized by vessel type.`;
        setSubTitle(subtitle);
      }
      else if(apiFilter==="harbor and type")
      {
        subtitle = `Fishing vessel data for the specified date range, providing insights into the vessel count that is , categorized by vessel type, present at the ports.`;
        setSubTitle(subtitle);
      }
  ...
  const fetchDataforActivityTrend = async () => {
    ...
    try {
      ...
      const response = await fetch(
        `${process.env.NEXT_PUBLIC_MSA_BACKEND_API}/activity_trend?date_from=${dateFrom}&&date_to=${dateTo}&&filter=${apiFilter}&&${queryString}`
      );
      if (!response.ok) {
    ...
    <>
      <div style={{ overflowY: "hidden" }}>
        ...
        {/* // )} */}
          ...
          <Button onClick={() => exportAsImage(exportRef.current, "Fishing-Activity-Trend", dateRange)}
        ...
        <Row ref={exportRef}>
          <Col span={24}>
            {loading ? (
              ...
              <Sunburst
                title="Fishing Activity - Sunburst"
                subTitle={subTitle}
                data={filteredData}
              />
            ) : ( <p>Loading chart...</p>) }
