  ...
  const fetchData = async () => {
    ...
    try {
      const response = await fetch(
        `${process.env.NEXT_PUBLIC_MSA_BACKEND_API}/mer_duration_at_sea?date_from=${dateFrom}&&date_to=${dateTo}`
      );
  ...
  return (
    <div>
      ...
      <div className="flex justify-end items-center p-6">
        ...
        <Button onClick={() => exportAsImage(exportRef.current, "Merchant-Sea-Duration", dateRange)}
      ...
      <Row ref={exportRef}>
        <Col span={24}>
          {loading ? (
            ...
            <PieChart3D
              title={"Merchant Vessel - Duration at Sea"}
              data={filteredData}
            />
          )}
        </Col>
